...
public class UmsMenuServiceImpl implements UmsMenuService {
    ...
    public List<UmsMenuNode> treeList() {
        ...
        List<UmsMenuNode> result = menuList.stream().filter(menu -> menu.getParentId().equals(0L)).map(menu -> covertMenuNode(menu, menuList)).collect(Collectors.toList());
        return result;
    }
    ...
    private void updateLevel(UmsMenu menu) {
        ...
        //有父菜单时选择根据父菜单Level设置
        UmsMenu umsMenu = menuMapper.selectByPrimaryKey(menu.getParentId());
        if (umsMenu != null) {
