...
public class UmsPermissionServiceImpl implements UmsPermissionService {
    ...
    public List<UmsPermissionNode> treeList() {
        ...
        List<UmsPermissionNode> result = permissionList.stream().filter(permission -> permission.getPid().equals(0L)).map(permission -> covert(permission, permissionList)).collect(Collectors.toList());
        return result;
    }
    ...
    public UmsPermissionNode covert(UmsPermission permission, List<UmsPermission> permissionList) {
        ...
        List<UmsPermissionNode> children = permissionList.stream().filter(subPermission -> subPermission.getPid().equals(permission.getId())).map(subPermission -> covert(subPermission, permissionList)).collect(Collectors.toList());
